...
function Task(options) {
  ...
  this.elitismSize = options.elitismSize || 20
...
Task.prototype.parentSelection = function (callback) {
  ...
  self.parents = []
    ...
    self.population.sort(function(p1, p2) {
  ...
  async.forEach(self.population,
    ...
    function (err) {
      if (err!=null) { self.emit('error', err) }
          ...
          // sort in ascending order, max fitness first
...
Task.prototype.childSelection = function (callback) {
  ...
  self.population = self.population.slice(0, self.elitismSize)
  while(self.population.length < self.popSize) {
      var c = self.children.pop();
